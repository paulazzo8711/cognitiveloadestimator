...
let directoryPath = vscode.workspace.getConfiguration("cognitiveloadestimator").get("directoryPath");
  ...
  if (mode === "API") {
      ...
      client.on('data', (data) => {
        if (data !== null && data !== undefined) { 
            ...
            const values = dataString.match(/"(.*?)"/g)?.map(value => parseFloat(value.replace(/"/g, ''))) ?? [];
